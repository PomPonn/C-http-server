...
void on_error() { error_last_print_message(); }
  ...
  switch (req->method) {
    case HTTP_GET: {
      ...
      if ((resource_size = read_file(full_path, &resource_content)) < 0) {
        ...
        // get mime type
        char* mime_type = pairs_str_get_value(
            mime_types, sizeof(mime_types) / sizeof(pair_str_t), extension);
        ...
        // add headers
        h_count =
            http_header_array_push(h_size, h_count, headers, 2,
                                   (http_header[]){
      ...
      *res = http_build_response(req->variant, req->version, status, headers,
                                 h_count, resource_content, &resource_size);

      if (headers) http_header_array_destroy(headers);

      if (resource_content) free_content(resource_content);
      break;
    }
  }
}
